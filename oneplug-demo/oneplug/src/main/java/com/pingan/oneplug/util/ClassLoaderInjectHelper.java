 ...
 * <p>
 * 这种方案也有不好的地方，安全性等；但是插件中通过intent中设置序列化对象extra时候，无法找到对应的类；只能通过此方法
 * </p>
 * <p/>
 * FIXME: 其中类框内只使用了部分函数，其他函数需要重新整理
 */
@SuppressLint("NewApi")
public class ClassLoaderInjectHelper {
     ...
     * 注入jar
     *
     * @param aApp     {@link Application}
     * @param aLibPath jar文件路径
     ...
     * 注入
     *
     * @param parentClassLoader 父{@link ClassLoader}
     * @param childClassLoader  子{@link ClassLoader}
     * @param someClass         某些需要特别处理的类
     * @return {@link InjectResult}
     * <p/>
     * FIXME: 框暂不使用
     */
     ...
     * @param sourceApk              插件apk文件
     * @param optimizedDirectory     优化后的dex文件目录
     * @param nativeLibraryDirectory 插件lib文件目录
     * @param someClass              某些需要特别处理的类
     * @return {@link InjectResult}
     */
    public static InjectResult inject(ClassLoader loader, File sourceApk, File optimizedDirectory,
                                      File nativeLibraryDirectory, String someClass) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= 19) {
                ...
                // FIXME: 框最小版本支持为14，V4的install未经过验证，api小于14时，沿用之前就的处理逻辑
                ClassLoader classLoader = new DexClassLoader(sourceApk.getAbsolutePath(),
                        optimizedDirectory.getAbsolutePath(), nativeLibraryDirectory.getAbsolutePath(),
                        loader);
                return inject(loader, classLoader, someClass);
            }
    ...
    private static InjectResult injectInAliyunOs(Application aApp, String aLibPath) {
        ...
        try {
            ...
            methodLoadClass.invoke(localLexClassLoader, "com.pingan.browser.favorite.BdInjectInvoker");
    ...
    private static InjectResult injectBelowApiLevel14(ClassLoader parentClassLoader,
        ...
        try {
            // 2.2版本以下需要触发DexClassLoader init
            ...
            try {
                ...
                ArrayList<String> libPaths = (ArrayList<String>) getField(pathClassLoader,
                        PathClassLoader.class, "libraryPathElements");
    ...
    private static InjectResult injectAboveEqualApiLevel14(Application aApp, String aLibPath) {
        ...
        InjectResult result = injectAboveEqualApiLevel14(pathClassLoader, dexClassLoader);
    ...
    private static InjectResult injectAboveEqualApiLevel14(ClassLoader parentClassLoader,
        ...
        try {
            ...
            Object nativeLibDirObj = getNativeLibraryDirectories(getPathList(dexClassLoader));
            boolean isListType = false;
            if (nativeLibDirObj instanceof List) {
                isListType = true;
            }

            // 注入native lib so目录，需要parent class loader遍历此目录能够找到。因为注入了以后，不处理这个目录找不到。
            if (!isListType) {
                Object dexNativeLibraryDirs = combineArray(
                        getNativeLibraryDirectories(getPathList(pathClassLoader)),
                ...
                try {
                    Object nativeLibraryPathElements = combineArray(
                            getNativeLibraryPathElements(getPathList(pathClassLoader)),
                            getNativeLibraryPathElements(getPathList(dexClassLoader)));

                    setField(pathList, pathList.getClass(), "nativeLibraryPathElements", nativeLibraryPathElements);
                } catch (Exception e) {
                    e.printStackTrace();
                }

                List nativeLibDirList = (List) nativeLibDirObj;

                expandFieldArray(pathList, "nativeLibraryDirectories", nativeLibDirList.toArray());
    ...
    private static final class V19 {
         ...
         *
         * @param loader                     {@link ClassLoader}
         * @param additionalClassPathEntries 插件代码文件
         * @param optimizedDirectory         已优化的文件目录
         * @param nativeLibraryDirectory     插件lib文件目录
         */
        private static void install(ClassLoader loader, List<File> additionalClassPathEntries,
                                    File optimizedDirectory, File nativeLibraryDirectory)
            ...
            expandFieldArray(dexPathList, "dexElements",
                    makeDexElements(dexPathList, new ArrayList<File>(additionalClassPathEntries),
            ...
            try {
                ArrayList<File> libDirList = new ArrayList<File>();
                libDirList.add(nativeLibraryDirectory);
                expandFieldArray(dexPathList, "nativeLibraryPathElements",
                        makePathElements(dexPathList, libDirList, optimizedDirectory, suppressedExceptions));
            } catch (Exception e) {
                e.printStackTrace();
        ...
        private static Object[] makePathElements(Object dexPathList, ArrayList<File> files,
            ...
            try {
                makePathElements = findMethod(dexPathList, "makePathElements", ArrayList.class, File.class,
                        ArrayList.class);
            } catch (NoSuchMethodException e) {
                makePathElements = null;
            }
            ...
            if (makePathElements == null) {
                try {
                    makePathElements = findMethod(dexPathList, "makePathElements", ArrayList.class,
            ...
            if (makePathElements == null) {
                try {
                    makePathElements = findMethod(dexPathList, "makePathElements", List.class, List.class);
                } catch (NoSuchMethodException e) {
                    makePathElements = null;
                }
            }
            if (makePathElements == null) {
                try {
                    makePathElements = findMethod(dexPathList, "makePathElements", List.class,
                            ArrayList.class);
                } catch (NoSuchMethodException e) {
                    makePathElements = null;
                }
    ...
    private static final class V4 {
        ...
        private static void install(ClassLoader loader, List<File> additionalClassPathEntries)
            ...
            for (ListIterator<File> iterator = additionalClassPathEntries.listIterator(); iterator
                    .hasNext(); ) {
     ...
     *
     * @param aResult 注入结果状态
     * @param aT      {@link Throwable}
     * @return {@link InjectResult}
     */
     ...
     * FIXME: 框暂时不使用
     */
    private static InjectResult ejectBelowApiLevel14(ClassLoader parentClassLoader,
                                                     ClassLoader childClassLoader) {
        InjectResult result = null;
        PathClassLoader pathClassLoader = (PathClassLoader) parentClassLoader;
        DexClassLoader dexClassLoader = (DexClassLoader) childClassLoader;
     ...
     * FIXME: 框暂不使用
     */
    private static InjectResult ejectAboveEqualApiLevel14(ClassLoader parentClassLoader,
                                                          ClassLoader childClassLoader) {
        PathClassLoader pathClassLoader = (PathClassLoader) parentClassLoader;
        DexClassLoader dexClassLoader = (DexClassLoader) childClassLoader;
        InjectResult result = null;
        try {
            ...
            // 注入native lib so目录，需要parent class loader遍历此目录能够找到。因为注入了以后，不处理这个目录找不到。
            Object dexNativeLibraryDirs = removeArrayElements(
                    getNativeLibraryDirectories(getPathList(pathClassLoader)),
                    getNativeLibraryDirectories(getPathList(dexClassLoader)));
